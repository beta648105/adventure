import { CHAR_W, CHAR_H, VIEW_W, VIEW_H, WALK_SPEED, WALK_FRAME_TIME } from '../config.js';
...
export class Player {
  ...
  update(dt) {
    ...
    // 화면 밖으로 나가지 않게
    this.x = Math.max(0, Math.min(VIEW_W - CHAR_W, this.x));
    this.y = Math.max(0, Math.min(VIEW_H - CHAR_H, this.y));
  }
